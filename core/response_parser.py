...
class ResponseParser:
    ...
    def _extract_explanation(self, text: str) -> str:
        """Extract the explanation text before structured sections"""
        # Find where structured section starts
        decision_match = self.decision_pattern.search(text)
        
        if decision_match:
            explanation = text[:decision_match.start()].strip()
        else:
            # If no structured section found, use first paragraph
